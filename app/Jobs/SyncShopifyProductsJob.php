...
/**
 * Full product sync for a single Shopify store.
 *
 * Queue:   sync-store
 * Timeout: 300 s (5 min) — large catalogues can have thousands of products
 * Tries:   3
 * Backoff: [60, 300, 900] s
 *
 * Fetches all Shopify products via GraphQL (cursor-paginated) and upserts them
 * into the products table. This is a full overwrite: every product is upserted
 * whether or not it changed (idempotent per store_id + external_id).
 *
 * ShopifyConnector::syncProducts() handles the pagination and upsert logic.
 *
 * Scheduled: daily at 02:00 UTC per active Shopify store.
 *
 * Related: app/Jobs/SyncProductsJob.php (WooCommerce equivalent)
...
class SyncShopifyProductsJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($this->isWorkspaceFrozen()) {
            Log::info('SyncShopifyProductsJob: skipped — workspace trial expired', [
        ...
        try {
            ...
            Log::info('SyncShopifyProductsJob: completed', [
            ...
            Log::error('SyncShopifyProductsJob: sync failed', [
                'store_id' => $this->storeId,
                'error'    => $e->getMessage(),
            ]);
